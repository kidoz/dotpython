...
#[unsafe(no_mangle)]
pub extern "C" fn dp_abi3_module_get_int(
    module: *mut PyObject,
    name: *const c_char,
    value: *mut i64,
) -> c_int {
    require_owner!(-1);
    if value.is_null() {
        set_error(unsafe { PyExc_TypeError }, "module query output is invalid");
        return -1;
    }
    let attribute = get_attribute_cstr(module, unsafe { cstr_to_str(name) });
    if attribute.is_null() {
        return -1;
    }
    // Clear any prior error so a legitimate -1 result is not misread as a failure.
    clear_error();
    let converted = PyLong_AsLong(attribute);
    release(attribute);
    if converted == -1 && !PyErr_Occurred().is_null() {
        return -1;
    }
    unsafe { *value = converted as i64 };
    0
}

#[unsafe(no_mangle)]
pub extern "C" fn dp_abi3_module_call_long(
    module: *mut PyObject,
    method: *const c_char,
    has_argument: c_int,
    argument: i64,
    ...
    require_owner!(-1);
    if result.is_null() {
        set_error(unsafe { PyExc_TypeError }, "module call output is invalid");
        return -1;
    }
    let callable = get_attribute_cstr(module, unsafe { cstr_to_str(method) });
    if callable.is_null() {
        return -1;
    }
    let args = PyTuple_New(if has_argument != 0 { 1 } else { 0 });
    if args.is_null() {
        release(callable);
        return -1;
    }
    if has_argument != 0 {
        if argument < c_long::MIN as i64 || argument > c_long::MAX as i64 {
            set_error(
                unsafe { PyExc_ValueError },
                "module argument does not fit in C long",
            );
            release(callable);
            release(args);
            return -1;
        }
        let number = PyLong_FromLong(argument as c_long);
        if number.is_null() || PyTuple_SetItem(args, 0, number) != 0 {
            release(callable);
            release(args);
            return -1;
        }
    ...
    if returned.is_null() {
        if with_err(|e| e.etype).is_null() {
            set_error(
                unsafe { PyExc_RuntimeError },
                "module method returned NULL without an error",
            );
        }
        return -1;
    }
    let converted = PyLong_AsLong(returned);
    release(returned);
    if converted == -1 && !PyErr_Occurred().is_null() {
        return -1;
    }
    unsafe { *result = converted as i64 };
    0
}
...
fn tuple_set_text(tuple: *mut PyObject, index: Py_ssize_t, text: &str) -> c_int {
    let value = new_unicode_str(text);
    if value.is_null() {
        return -1;
    }
...
fn call_named(owner: *mut PyObject, name: &str, args: *mut PyObject) -> *mut PyObject {
    let callable = get_attribute_cstr(owner, name);
    if callable.is_null() {
        return ptr::null_mut();
    }
    let result = PyObject_Call(callable, args, ptr::null_mut());
...
#[unsafe(no_mangle)]
pub extern "C" fn dp_abi3_anyver_compare(
    module: *mut PyObject,
    left: *const c_char,
    right: *const c_char,
    ...
    require_owner!(-1);
    if left.is_null() || right.is_null() || ecosystem.is_null() || result.is_null() {
        set_error(
            unsafe { PyExc_TypeError },
            "Anyver comparison inputs are invalid",
        );
        return -1;
    }
    let args = PyTuple_New(3);
    if args.is_null()
        || tuple_set_text(args, 0, unsafe { cstr_to_str(left) }) != 0
        || tuple_set_text(args, 1, unsafe { cstr_to_str(right) }) != 0
        || tuple_set_text(args, 2, unsafe { cstr_to_str(ecosystem) }) != 0
    {
        release(args);
        return -1;
    }
    clear_error();
    let returned = call_named(module, "compare", args);
    release(args);
    if returned.is_null() {
        return -1;
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn dp_abi3_anyver_sort_versions(
    module: *mut PyObject,
    ...
    require_owner!(-1);
    if versions.is_null()
        || !(0..=4096).contains(&version_count)
        || ecosystem.is_null()
        || result_json.is_null()
    {
        set_error(unsafe { PyExc_TypeError }, "Anyver sort inputs are invalid");
        return -1;
    }
    ...
    for index in 0..version_count {
        let entry = unsafe { *versions.offset(index as isize) };
        if entry.is_null() {
            release(list);
            set_error(
                unsafe { PyExc_TypeError },
                "Anyver sort contains a NULL version",
            );
            return -1;
        }
        let value = new_unicode_str(unsafe { cstr_to_str(entry) });
        if value.is_null() || PyList_Append(list, value) != 0 {
            release(value);
            release(list);
            return -1;
        }
    ...
    if args.is_null()
        || PyTuple_SetItem(args, 0, list) != 0
        || tuple_set_text(args, 1, unsafe { cstr_to_str(ecosystem) }) != 0
    {
        release(args);
        return -1;
    }
    clear_error();
    let sorted = call_named(module, "sort_versions", args);
    release(args);
    if sorted.is_null() {
        return -1;
    }
    let count = PyList_Size(sorted);
    if count < 0 {
        release(sorted);
        return -1;
    }
    ...
    for index in 0..count {
        let text = PyUnicode_AsUTF8AndSize(PyList_GetItem(sorted, index), ptr::null_mut());
        if text.is_null() {
            release(sorted);
            return -1;
        }
        ...
        let bytes = unsafe { core::ffi::CStr::from_ptr(text) }
            .to_bytes()
            .to_vec();
    ...
    if stored.is_null() {
        set_error(
            unsafe { PyExc_ValueError },
            "Anyver sort result exceeds the bridge limit",
        );
        return -1;
    }
...
fn dict_value(dictionary: *mut PyObject, name: &str) -> *mut PyObject {
    let dkey = new_unicode_str(name);
    if dkey.is_null() {
        return ptr::null_mut();
    }
    let value = PyDict_GetItemWithError(dictionary, dkey);
    release(dkey);
    value
}

#[unsafe(no_mangle)]
pub extern "C" fn dp_abi3_anyver_version_to_json(
    module: *mut PyObject,
    ...
    require_owner!(-1);
    if version.is_null() || ecosystem.is_null() || result_json.is_null() {
        set_error(
            unsafe { PyExc_TypeError },
            "Anyver Version inputs are invalid",
        );
        return -1;
    }
    let type_ = get_attribute_cstr(module, "Version");
    let args = PyTuple_New(2);
    if type_.is_null()
        || args.is_null()
        || tuple_set_text(args, 0, unsafe { cstr_to_str(version) }) != 0
        || tuple_set_text(args, 1, unsafe { cstr_to_str(ecosystem) }) != 0
    {
        release(type_);
        release(args);
    ...
    clear_error();
    let instance = PyObject_Call(type_, args, ptr::null_mut());
    release(type_);
    release(args);
    if instance.is_null() {
        return -1;
    ...
    let dictionary = if empty.is_null() {
        ptr::null_mut()
    } else {
        call_named(instance, "to_dict", empty)
    };
    release(empty);
    release(instance);
    ...
    let raw = dict_value(dictionary, "raw");
    let eco = dict_value(dictionary, "ecosystem");
    let build = dict_value(dictionary, "build");
    let epoch = dict_value(dictionary, "epoch");
    let major = dict_value(dictionary, "major");
    let minor = dict_value(dictionary, "minor");
    let patch = dict_value(dictionary, "patch");
    let prerelease = dict_value(dictionary, "is_prerelease");
    let postrelease = dict_value(dictionary, "is_postrelease");

    let raw_text = PyUnicode_AsUTF8AndSize(raw, ptr::null_mut());
    let eco_text = PyUnicode_AsUTF8AndSize(eco, ptr::null_mut());
    let build_text = PyUnicode_AsUTF8AndSize(build, ptr::null_mut());
    if raw_text.is_null()
        || eco_text.is_null()
        || build_text.is_null()
        || epoch.is_null()
    ...
    let epoch_value = PyLong_AsLong(epoch);
    let major_value = PyLong_AsLong(major);
    let minor_value = PyLong_AsLong(minor);
    let patch_value = PyLong_AsLong(patch);
    if !PyErr_Occurred().is_null() {
        release(dictionary);
        return -1;
    }
    let is_true = |p: *mut PyObject| p == unsafe { &raw mut _Py_TrueStruct };
    let bytes_of = |p: *const c_char| unsafe { core::ffi::CStr::from_ptr(p) }.to_bytes().to_vec();

    let mut json = String::from("{\"raw\":");
    json_escape(&bytes_of(raw_text), &mut json);
    json.push_str(",\"ecosystem\":");
    json_escape(&bytes_of(eco_text), &mut json);
    json.push_str(&format!(",\"epoch\":{epoch_value}"));
    json.push_str(&format!(",\"major\":{major_value}"));
    json.push_str(&format!(",\"minor\":{minor_value}"));
    json.push_str(&format!(",\"patch\":{patch_value}"));
    json.push_str(",\"build\":");
    json_escape(&bytes_of(build_text), &mut json);
    json.push_str(",\"is_prerelease\":");
    json.push_str(if is_true(prerelease) { "true" } else { "false" });
    json.push_str(",\"is_postrelease\":");
    json.push_str(if is_true(postrelease) {
        "true"
    } else {
        "false"
    });
    ...
    if stored.is_null() {
        set_error(
            unsafe { PyExc_ValueError },
            "Anyver Version result exceeds the bridge limit",
        );
        return -1;
    }
    unsafe { *result_json = stored };

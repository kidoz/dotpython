//! Minimal DotPython Stable-ABI conformance fixture (Rust port of `dotpython_fixture.c`).
//!
//! An independent C-ABI consumer of the bridge: it declares the bridge functions as `extern` and
//! is loaded after the bridge, so its undefined `Py*` symbols resolve at load time. It therefore
//! keeps proving genuine C-ABI compatibility, exactly as the C fixture did.
...
unsafe extern "C" fn fixture_increment(
    _self: *mut PyObject,
    ...
    unsafe { PyLong_FromLong(value + 1) }
}

unsafe extern "C" fn fixture_fail(_self: *mut PyObject, _ignored: *mut PyObject) -> *mut PyObject {
    unsafe { PyErr_SetString(PyExc_ValueError, c"fixture failure".as_ptr()) };
    ptr::null_mut()
}

unsafe extern "C" fn fixture_execute(module: *mut PyObject) -> c_int {
    unsafe { PyModule_AddIntConstant(module, c"fixture_ready".as_ptr(), 1) }
}
...
static mut METHODS: [PyMethodDef; 3] = [
    PyMethodDef {
        ml_name: c"increment".as_ptr(),
        ml_meth: Some(fixture_increment),
        ml_flags: METH_O,
        ml_doc: c"Increment a whole number.".as_ptr(),
    },
    PyMethodDef {
        ml_name: c"fail".as_ptr(),
        ml_meth: Some(fixture_fail),
        ml_flags: METH_NOARGS,
        ml_doc: c"Raise a deliberate fixture error.".as_ptr(),
    },
    PyMethodDef {
        ml_name: ptr::null(),
        ml_meth: None,
...
static mut DEFINITION: PyModuleDef = PyModuleDef {
    ...
    m_name: c"dotpython_fixture".as_ptr(),
    m_doc: c"Minimal DotPython Stable-ABI fixture.".as_ptr(),
    m_size: 0,
...
pub extern "C" fn PyInit_dotpython_fixture() -> *mut PyObject {
    unsafe {
        ...
        DEFINITION.m_slots = &raw mut SLOTS as *mut PyModuleDef_Slot;
